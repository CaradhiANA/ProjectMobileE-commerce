...
class _CartPageState extends State<CartPage> with TickerProviderStateMixin {
  ...
  Future<void> fetchItems() async {
    ...
    final data = await json.decode(response);
     ...
     cartItems = data['products']
       .map((data) => Barang.fromJson(data)).toList();      
  ...
  cartItem(Barang barang, int index, animation) {
    return GestureDetector(
      onTap: () {
        Navigator.push(context, MaterialPageRoute(builder: (context) => ProductViewPage(barang: barang)));
      },
            ...
            children: <Widget>[
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                     Text(barang.brand, style: TextStyle(color: Colors.orange.shade400, fontSize: 14,),),
                     SizedBox(height: 5,),
                     Text(barang.name,
                       style: TextStyle(
                         fontSize: 18,
                    ...
                    onPressed: () {
                      setState(() {
                        if (cartItemCount[index] > 1) {
                          cartItemCount[index]--;
                          totalPrice = totalPrice - barang.price;
                        }
                      });
                    },
                    shape: CircleBorder(),
